...
static int writePIC(const QRcode *qrcode, const char *outfile)
{
	...
	fprintf(fp,  "command \".if !mqrcode_fg .defcolor qrcode_fg rgb #%s\"\n", fg);
	fprintf(fp,  "command \".if !mqrcode_bg .defcolor qrcode_bg rgb #%s\"\n", bg);
	if (svg_path) {
		fputs("command \".nop \\X'ps: exec 0 setlinecap'\\c\"\n", fp);
		fputs("command \".nop \\X'ps: exec 0 setlinejoin'\\c\"\n", fp);
		fprintf(fp, "linethick=%d\n", size);
	}

	fprintf(fp, "maxpswid=%1$d;maxpsht=%1$d;movewid=0;moveht=1;boxwid=1;boxht=1\n", symwidth);
	if(svg_path) {
		fputs("define P { line left $3 color $5 at $1+4,-$2-4 }\n", fp);
	} else {
		fputs("define P { box wid $3 ht $4 color $5 with .nw at $1+4,-$2-4 }\n", fp);
	}
  fputs("box wid maxpswid ht maxpsht color \"qrcode_bg\" with .nw at 0,0\n", fp);
	...
	for(y = 0; y < qrcode->width; y++) {
		...
		for(x = 0; x < qrcode->width; x++) {
			if(*(row+x)&0x1) {
				writePIC_drawModules(fp, x, y, 1, "qrcode_fg");
			}
		}
	}
